...
print train.describe()   # only numeric data is displayed only
...
print train.describe(include="all")   # to display data of all
...
#   unique          NaN       NaN     NaN                        891     2     NaN     NaN     NaN       681     NaN          147        3   to get the catogorial value
#   top             NaN       NaN     NaN  Graham, Mr. George Edward  male     NaN     NaN     NaN  CA. 2343     NaN  C23 C25 C27        S   one with most frequency
#   freq            NaN       NaN     NaN                          1   577     NaN     NaN     NaN         7     NaN            4      644   his frequency
...
print train.describe(include='all').T.unique
...
#Alphanumeric Features: Name, Ticket, Cabin   can't be used in ML
...
sbn.barplot(x="Sex", y="Survived", data=train)   # seaborn extension of matplotlib
plt.show()

# The raatio is given to the total no of male/female present
...
print("Percentage of Pclass = 1 who survived:", train["Survived"][train["Pclass"] == 1].value_counts(normalize = True)[1]*100)  # counting is done
...
# using normalize tells percentage instead of just the numbers
# by default it prints in descending order
...
# Feature Engineering/Feature Extraction - To create a new feature by using some operation on existing feature

#sort the ages into logical categories
train["Age"] = train["Age"].fillna(-0.5)   # here filling the missing value to -0.5
test["Age"] = test["Age"].fillna(-0.5)

bins =   [-1, 0, 5, 12, 18, 24, 35, 60, np.inf]   # np.inf - positive infinite values
labels = ['Unknown', 'Baby', 'Child', 'Teenager', 'Student', 'Young Adult', 'Adult', 'Senior']
train['AgeGroup'] = pd.cut(train["Age"], bins, labels = labels)
# cut function(para1:taking of data, para2:matching of data, para3:finally fitting the value )
...
train = train.drop(['Cabin'], axis = 1)  # column
...
for dataset in combine:
    ...
    # str.extract(' ([A-Za-z]+)\.',)  == Regular Expression meaning starting from space and ending at . with an
    #                                    alphabet in between   (+,* same as FLA)
    #                                    \. to take . as text not as expression
    # expand = false (Don't search more than once)
...
# crosstab(report made against this column, report made with this column) - p1 : rows , p2 : cols
...
# as_index=false - index won't be printed in the output
...
for dataset in combine:
    dataset['Title'] = dataset['Title'].map(title_mapping)   # mapping to this dictionary
    dataset['Title'] = dataset['Title'].fillna(0)               # filling 0 to people with no title
...
print "\n\n********   train[AgeGroup][x] :  \n\n"
for x in range(10) :
    print train["AgeGroup"][x]    # print first 10 values of age group column
...
for x in range(len(train["AgeGroup"])):
    if train["AgeGroup"][x] == "Unknown":   # x=5 ( means for 6th record )
        ...
        # replacing unknowns from the age group according to the title
...
for x in range(len(test["Fare"])):
    if pd.isnull(test["Fare"][x]):
        ...
        test["Fare"][x] = round(train[ train["Pclass"] == pclass ]["Fare"].mean(), 2)   # round off till 2 decimal place

#map Fare values into groups of numerical values
train['FareBand'] = pd.qcut(train['Fare'], 4, labels = [1, 2, 3, 4])
test['FareBand'] = pd.qcut(test['Fare'], 4, labels = [1, 2, 3, 4]) # excluding the end conditions
